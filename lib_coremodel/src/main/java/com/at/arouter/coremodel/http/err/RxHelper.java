package com.at.arouter.coremodel.http.err;
    ...
    public static <T> ObservableTransformer<RequestResult<T>, T> handleResult() {
        return upstream -> upstream.flatMap(requestResult -> {
            ...
            if (requestResult.code == RequestResult.CODE_REQUEST_SUCCESS) {
                ...
                return createSuccessData(requestResult.result);
            } else {
                //非 code = 1 情况弹出提示
                Exception e = new Exception(requestResult.msg, new Throwable(ExceptionUtils.CodeUtil.MSG + ""));
    ...
    public static <T> ObservableTransformer<RequestResult<T>, T> handleResultList() {
        return upstream -> upstream.flatMap(requestResult -> {
            ...
            if (requestResult.code == RequestResult.CODE_REQUEST_SUCCESS) {
                ...
                if ((requestResult.result instanceof ArrayList && ((ArrayList) requestResult.result).size() == 0)
                        || (requestResult.result instanceof List && ((List) requestResult.result).size() == 0)) {
