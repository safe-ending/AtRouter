...
public class CustomEditText2 extends LinearLayout {
    ...
    public CustomEditText2(Context context, @Nullable AttributeSet attrs) {
        ...
        isDisplayRightImg = ta.getBoolean(R.styleable.CustomEditText2_display_right_icon2, false);
        ...
        textSize = ta.getDimensionPixelSize(R.styleable.CustomEditText2_text_size2, 15);
        textHintColor = ta.getColor(R.styleable.CustomEditText2_text_hint_color, ContextCompat.getColor(context, R.color.bg_999999));
        textColor = ta.getColor(R.styleable.CustomEditText2_text_color, ContextCompat.getColor(context, R.color.color_000000));
        ...
        if (isDisplayRightImg) {
            mRightImg.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View v) {
                    if (mEtContent.getTransformationMethod() != HideReturnsTransformationMethod.getInstance()) {
                        mEtContent.setTransformationMethod(HideReturnsTransformationMethod.getInstance());
                        ivEye.setBackgroundResource(R.mipmap.yanjing1);
                        ...
                        mEtContent.setSelection(mEtContent.getText().toString().length());
                        ivEye.setBackgroundResource(R.mipmap.yanjing11);
                    }
                }
            });
    ...
    private void initView() {
        ...
        mRightImg.setVisibility(isDisplayRightImg ? VISIBLE : GONE);
    ...
    public void setPswdOnly(int lenght) {
        if (mEtContent == null)
            return;
        mEtContent.setFilters(new InputFilter[]{new InputFilter.LengthFilter(lenght)});//限定长度
        mEtContent.setKeyListener(DigitsKeyListener.getInstance("0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ"));
        mEtContent.setTransformationMethod(PasswordTransformationMethod.getInstance());//从密码可见模式变为密码不可见模式
    }

    public void setNumberPswdMode(int lenght) {
        if (mEtContent == null)
            return;
        mEtContent.setFilters(new InputFilter[]{new InputFilter.LengthFilter(lenght)});//限定长度
        //只能是数字，密码模式
        mEtContent.setKeyListener(DigitsKeyListener.getInstance("0123456789"));
        mEtContent.setTransformationMethod(PasswordTransformationMethod.getInstance());//从密码可见模式变为密码不可见模式
    }
