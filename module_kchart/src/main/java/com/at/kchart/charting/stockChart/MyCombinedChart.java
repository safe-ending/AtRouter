    ...
    protected void drawMarkers(Canvas canvas) {
        ...
        for (int i = 0; i < mIndicesToHighlight.length; i++) {
            ...
            if (this.timeType == TimeType.TIME_HOUR) {
                date = DataTimeUtil.secToTime(kLineData.getKLineDatas().get((int) e.getX()).getDateTime() * 1000);
            } else {
                date = DataTimeUtil.secToDate(kLineData.getKLineDatas().get((int) e.getX()).getDateTime() * 1000);
            }
