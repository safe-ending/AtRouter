                ...
                for (int i = 0; i < data.length(); i++) {
                    ...
                    if (i == 0) {
                        ...
                        if (fiveDayXLabelKey.size() > index) {
                            fiveDayXLabels.put(fiveDayXLabelKey.get(index), DataTimeUtil.secToDateForFiveDay(timeDatamodel.getTimeMills()));
                            index++;
                        }
                    } else {
                        mAllVolume += timeDatamodel.getVolume();
                        if (fiveDayXLabelKey.size() > index && !DataTimeUtil.secToDateForFiveDay(timeDatamodel.getTimeMills()).equals(preDate)) {
                            fiveDayXLabels.put(fiveDayXLabelKey.get(index), DataTimeUtil.secToDateForFiveDay(timeDatamodel.getTimeMills()));
                            index++;
                        }
                    }
                    preDate = DataTimeUtil.secToDateForFiveDay(timeDatamodel.getTimeMills());
