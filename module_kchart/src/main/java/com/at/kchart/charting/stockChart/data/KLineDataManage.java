package com.at.kchart.charting.stockChart.data;
...
import com.at.kchart.charting.data.BarDataSet;
import com.at.kchart.charting.data.CandleEntry;
import com.at.kchart.charting.data.LineData;
import com.at.kchart.charting.data.LineDataSet;
import com.at.kchart.charting.interfaces.datasets.ILineDataSet;
import com.at.kchart.charting.stockChart.model.bean.BOLLEntity;
import com.at.kchart.charting.stockChart.model.bean.MACDEntity;
import com.at.kchart.charting.stockChart.model.bean.RSIEntity;
import com.at.kchart.charting.utils.DataTimeUtil;
import com.at.kchart.charting.data.BarEntry;
import com.at.kchart.charting.data.CandleDataSet;
import com.at.kchart.charting.data.Entry;
import com.at.kchart.charting.stockChart.model.bean.KDJEntity;
...
public class KLineDataManage {
     ...
     * 解析K线数据
     */
    public void parseKlineData(ArrayList<KLineDataModel> object, String assetId, boolean landscape) {
...
//                    object.get(i).set
//                    klineDatamodel.setDateTime(data.optJSONArray(i).optLong(0, 0L));
...
//                    klineDatamodel.setVol(NumberUtils.stringNoE10ForVol(Double.isNaN(data.optJSONArray(i).optDouble(5)) ? 0 : data.optJSONArray(i).optDouble(5)));
...
//                    klineDatamodel.setPreClose(data.optJSONArray(i).optDouble(12));
                if (i+1<object.size()){//收盘价为第二天的开盘价
                    object.get(i).setClose(object.get(i+1).getOpen());
                }

                object.get(i).setPreClose(object.get(i).getClose());
                object.get(i).setMa5(getMa(5, object, i));
                object.get(i).setMa10(getMa(10, object, i));
                object.get(i).setMa20(getMa(20, object, i));
                klineDatamodel = object.get(i);
                preClosePrice = klineDatamodel.getClose();
                kDatas.add(klineDatamodel);

                xVal.add(DataTimeUtil.secToDate(getKLineDatas().get(i).getDateTime() * 1000));
                ...
                barEntries.add(new BarEntry(i, i + offSet, (float) getKLineDatas().get(i).getVol(), color));
    ...
    //获取5分钟 ma5  即前5天平均值
    public float getMa(int type, ArrayList<KLineDataModel> data, int i) {
        if (i >= 0) {
            List<Float> floatList = new ArrayList<>();
            for (int j = 0; j < type; j++) {
                if (i >= 0) {
                    floatList.add((float) data.get(i--).getClose());
                } else {
                    break;
                }
            }
            ...
            for (int k = 0; k < floatList.size(); k++) {
                maValue += floatList.get(k);
            }
            float ff = new BigDecimal(maValue).divide(new BigDecimal(floatList.size()), 4, BigDecimal.ROUND_DOWN).floatValue();
//            Log.e("11",ff+"");
            return ff;
        }
